...
class ElecAllView(APIView):
    # 查询数据请求接口
    def get(self, request):
        # 获取所有数据
        alldata = models.ElectronicModel.objects.all()
        # 序列化所有数据 [instance=所有数据]
        # many=True 参数仅在序列化查询集（即多个对象）时使用
        ser = serializers.ElecSerializer(instance=alldata, many=True)
        # 返回序列化后的数据
    ...
    def post(self, request):
        ...
        ser = serializers.ElecSerializer(data=request.data)
        # 验证数据合规性
        # 合规-save() 不合规-errors
        if ser.is_valid():
            ser.save() # 自动处理创建
    ...
    def put(self, request, id):
        try:
            data = models.ElectronicModel.objects.get(id=id)
        except models.ElectronicModel.DoesNotExist:
            raise NotFound(detail='要修改的数据不存在!')
        # instance 获取数据库中现有数据  data=request.data 获取修改后的数据
        # 两个条件必须同时写入时 才可执行update()方法 否则为create()操作
        ser = serializers.ElecSerializer(instance=data, data=request.data)
        if ser.is_valid():
            ser.save() # 触发保存
            return Response(ser.data, status=201)
        else:
    ...
    def delete(self, request, id):
        try:
            elecdel = models.ElectronicModel.objects.get(id=id).delete()
            if elecdel:
...
# 该视图接口仅用于查找单条数据
class ElecSelectView(APIView):
    def get(self, request, id): # 根据ID查找单条数据
        # 根据ID查找对应数据
        selectdata = models.ElectronicModel.objects.get(id=id)
        # 序列化查找到的数据
        # many=True 参数仅在序列化查询集（即多个对象）时使用
        ser = serializers.ElecSerializer(instance=selectdata)
        # 返回序列化后的数据
        return Response(ser.data, status=200)